...
function HeroSection() {
  return (
    <section className="relative min-h-[90vh] bg-gradient-to-br from-blue-900 via-blue-800 to-[#D6EBFF] text-white overflow-hidden">
      <div className="max-w-7xl mx-auto px-6 py-24 grid grid-cols-1 lg:grid-cols-2 gap-16 items-center relative z-10">

        <div>
          <span className="inline-block px-4 py-1 rounded-full bg-white/10 text-xs tracking-widest uppercase mb-6">
            About Our Pharmacy
          </span>
          <h1 className="text-5xl md:text-7xl font-black leading-tight">
            Healthcare<br />
            Built On <span className="text-[#D6EBFF]">Trust</span>
          </h1>
          <p className="mt-6 max-w-xl text-blue-100 text-lg leading-relaxed">
            We combine clinical excellence with compassionate service to deliver personalized pharmaceutical care.
          </p>
        </div>

        <div className="relative">
          <div className="absolute -inset-4 bg-white/10 rounded-[2.5rem] blur-2xl" />
          <div className="relative aspect-[4/3] rounded-[2.5rem] overflow-hidden border-4 border-white shadow-2xl">
            <Image src="/1.jpg" alt="About" fill className="object-cover" />
          </div>
        </div>
      </div>
    </section>
...
function StatsStrip() {
  const stats = [
    { icon: Users, number: "119K+", label: "Patients" },
    { icon: Pill, number: "29+", label: "Services" },
    { icon: CheckCircle, number: "149K+", label: "Solved" },
    { icon: Stethoscope, number: "199", label: "Doctors" },
  ]
  ...
  return (
    <section className="-mt-24 relative z-20">
      <div className="max-w-7xl mx-auto px-6 grid grid-cols-2 md:grid-cols-4 gap-6">
        {stats.map((s, i) => (
          <div key={i} className="bg-white/80 backdrop-blur-xl rounded-2xl shadow-xl border border-blue-100 p-6 text-center">
            <s.icon className="mx-auto text-blue-600 mb-3" />
            <div className="text-2xl font-black text-blue-900">{s.number}</div>
            <div className="text-xs uppercase text-slate-500 font-semibold">{s.label}</div>
          </div>
        ))}
      </div>
    </section>
...
function ServicesRail() {
  const services = [
    { icon: Leaf, title: "Vitamins", desc: "Daily nutritional care" },
    { icon: Building, title: "Generic Plans", desc: "Affordable alternatives" },
    { icon: Heart, title: "Vaccines", desc: "Immunity protection" },
  ]
    ...
    <section className="py-24 bg-white">
      <div className="max-w-7xl mx-auto px-6 grid lg:grid-cols-2 gap-16 items-center">
        ...
        <div>
          ...
          <p className="text-blue-800/70 max-w-xl">
            Comprehensive pharmaceutical services designed to simplify and protect your healthcare journey.
          </p>
        </div>

        <div className="space-y-6">
          {services.map((s, i) => (
            <div key={i} className="group flex items-center gap-5 p-6 rounded-2xl border border-blue-100 bg-[#E8F4FF]/60 hover:bg-white transition shadow-sm hover:shadow-lg">
              <div className="p-3 bg-[#D6EBFF] rounded-xl text-blue-700">
                <s.icon />
              </div>
              <div>
                <h4 className="font-black text-blue-900">{s.title}</h4>
...
// ================= WHY US TIMELINE
function WhyUsTimeline() {
  return (
    <section className="py-24 bg-[#D6EBFF]/30">
      <div className="max-w-7xl mx-auto px-6 grid lg:grid-cols-2 gap-20 items-center">

        <Image src="/3.gif" alt="Why Us" width={600} height={400} className="rounded-[2rem] shadow-2xl border-4 border-white" />

        <div>
          <h2 className="text-5xl font-black text-blue-900 mb-8">
            Why Patients Choose <span className="text-blue-600">Us</span>
          </h2>

          <div className="space-y-6">
            {[
              "Personalized medication therapy management",
              "Free, fast & confidential delivery",
  ...
  return (
    <section className="py-32 bg-white relative overflow-hidden">
      <div className="max-w-7xl mx-auto px-6 grid lg:grid-cols-2 gap-24 items-center">
        ...
        <div>
          <span className="inline-flex items-center gap-2 text-xs font-bold text-blue-700 bg-blue-100 px-3 py-1 rounded-md mb-6">
            <ShieldCheck className="w-4 h-4" /> QUALITY ASSURED
          </span>

          <h2 className="text-6xl font-black text-blue-950 mb-6">
            Clinical <span className="text-blue-600">Excellence</span>
          </h2>

          <p className="text-lg text-slate-600 max-w-xl">
            Our clinical pharmacists deliver science-backed care with a patient-first approach.
          </p>

          <div className="mt-10 flex gap-4">
            <div className="flex items-center gap-3 bg-[#E8F4FF] px-5 py-3 rounded-xl">
              <Award className="text-blue-600" /> Certified Standards
            </div>
            <div className="flex items-center gap-3 bg-[#E8F4FF] px-5 py-3 rounded-xl">
              <Search className="text-blue-600" /> Precision Verified
            </div>
          </div>
        </div>

        <div className="relative aspect-[4/5] rounded-[2.5rem] overflow-hidden shadow-2xl border-4 border-white">
          <Image src="/r.png" alt="Pharmacist" fill className="object-cover object-top" />
